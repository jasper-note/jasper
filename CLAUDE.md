...
- **编辑期钩子（0.4 阶段 4）**：manifest `[[contributes.editor]]{on}`（`on`∈before-save|input，需 `[backend]`）→ 编辑期把编辑缓冲文本交给 wasm 的 `editor.transform`（spec §3.7/§6.5/§8）改写。宿主 `POST /api/plugins/{id}/editor/transform` 以 `{phase,text}` 调、返回 `{text}`——**纯文本 in/out，走无 notes/ai 上下文的 `PluginHost::dispatch`**（notes.*→unsupported，防写入重入），任一插件失败即跳过（不丢输入）。前端**仅源码编辑器**（`Editor.svelte`）接 `input` 相位：CodeMirror debounce 输入停顿后依次调 `editorInputPlugins()`（`plugins.svelte.ts`）里各插件、保守替换缓冲——**仅真实用户输入**（`tr.isUserEvent('input'|'delete')`）触发（程序化 dispatch/外部同步不触发 → 天然防环）、等待期间用户又敲字（当前 doc≠送出文本）则丢弃变换（不覆盖新输入）；富文本不接（整篇重排风险高）。`before-save` 相位前端接入 + spec §3.7 可选 `command` 复用后置（现有服务端 `hook.before_save` 已覆盖保存前改写）。SDK 槽签名 `editor: fn(&str /*phase*/, String /*text*/)->Result<String,PluginError>`。宿主链路夹具 = testbed 的 `editor.transform`（标相位+全大写，`routes.rs::editor_transform_end_to_end`）。
